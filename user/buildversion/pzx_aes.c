...
static inline unsigned char get_sbox_value(unsigned int num)
{
    return sbox[num];
}

static inline unsigned char get_sbox_invert_value(unsigned int num)
{
    return rsbox[num];
}

// internal function, do not think about dst is same as src
static inline void aes_memcpy(void *dst, const void *src, unsigned int size)
{
    char *d8 = (char *)dst;
    char *s8 = (char *)src;
    while(size--)
...
static inline void add_round_key(unsigned char round, state_t *state, const unsigned char *round_key)
{
...
static inline void inverse_sub_bytes(state_t *state)
{
    unsigned char i = 0, j = 0;
    for(i = 0; i < 4; i++)
    {
        for(j = 0; j < 4; j++)
        {
            (*state)[j][i] = get_sbox_invert_value((*state)[j][i]);
        }
    }
}
...
static inline void inverse_shift_rows(state_t *state)
{
    unsigned char tmp = (*state)[3][1];
    (*state)[3][1] = (*state)[2][1];
    (*state)[2][1] = (*state)[1][1];
    (*state)[1][1] = (*state)[0][1];
    (*state)[0][1] = tmp;
...
{
    unsigned char i = 0, tmp = 0, tm = 0, t = 0;
    for(i = 0; i < 4; i++)
    {
        t = (*state)[i][0];
...
static void inverse_mix_columns(state_t *state)
{
    unsigned char i = 0, a = 0, b = 0, c = 0, d = 0;
    for(i = 0; i < 4; i++)
    {
        a = (*state)[i][0];
        b = (*state)[i][1];
...
static void inverse_cipher(state_t *state, const unsigned char *round_key)
{
    unsigned char round = 0;
...
void aes_init_ctx_iv(struct aes_ctx *ctx, const unsigned char *key, const unsigned char *iv)
{
    ...
    aes_memcpy(ctx->iv, iv, AES_BLKLEN);
}
...
static inline void xor_with_iv(unsigned char *buf, const unsigned char *iv)
{
    for(unsigned char i = 0; i < AES_BLKLEN; i++)
    {
        buf[i] ^= iv[i];
    }
}
...
{
    unsigned int i = 0;
    unsigned char next_iv[AES_BLKLEN] = {0};

    for(i = 0; i < len; i += AES_BLKLEN)
    {
        aes_memcpy(next_iv, buf, AES_BLKLEN);
        inverse_cipher((state_t *)buf, ctx->round_key);
        xor_with_iv(buf, ctx->iv);
        aes_memcpy(ctx->iv, next_iv, AES_BLKLEN);
        buf += AES_BLKLEN;
    }
}